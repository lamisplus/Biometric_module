...
public class BiometricTemplateIndex {
    ...
    /**
     * Reads candidates straight from the source of truth, applying the same length filter and
     * ordering the index would have applied.
     *
     * Returning an empty list when the index is unusable would let a capture report no match, which
     * silently accepts a duplicate fingerprint and makes identification fail; matching has to stay
     * correct even when the optimisation behind it is unavailable, so this is the slow path rather
     * than an error.
     */
